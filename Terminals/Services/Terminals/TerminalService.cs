...
using ContractRegionDto = global::Terminals.Web.Contracts.RegionDto;
using ContractTerminalRegionDto = global::Terminals.Web.Contracts.TerminalRegionDto;
...
public sealed class TerminalService : IRegionService
{
    ...
    async Task<IReadOnlyList<ContractRegionDto>> IRegionService.GetRegionsAsync(CancellationToken ct)
        => await _db.Regions
            .AsNoTracking()
            .OrderBy(r => r.Id)
            .Select(r => new ContractRegionDto(r.Id, r.Name))
            .ToListAsync(ct);

    async Task<IReadOnlyList<ContractTerminalRegionDto>> IRegionService.GetRegionTerminalsAsync(int regionId, CancellationToken ct)
        => await _db.TerminalRegionMaps
            .Where(x => x.RegionId == regionId)
            .OrderBy(x => x.TerminalId)
            .Select(x => new ContractTerminalRegionDto(x.TerminalId, x.RegionId))
            .ToListAsync(ct);
